        ...
        if modo == 'titulo':
            ...
            for task in analysis_crew.tasks:
                if task.description in task_map:
                    ...
                    # AQUI ESTÁ A CORREÇÃO: Usamos 'caminho_repo' em vez de 'self.BASE_DIR'
                    file_path = os.path.join(caminho_repo, filename)
                    with open(file_path, "w", encoding="utf-8") as f:
                        f.write(task.output.raw_output)
                    print(f"   - Arquivo '{filename}' salvo em '{caminho_repo}'.", file=sys.stderr)
            
            resumo_agent = codewise_instance.summary_specialist()
            resumo_task = Task(
                description="Com base no contexto da análise completa fornecida, crie um 'Resumo Executivo do Pull Request' **obrigatoriamente em Português do Brasil**, bem formatado em markdown.",
